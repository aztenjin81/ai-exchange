# Full source dump (part 1 of 2)

**Generated:** 2026-05-18 16:42:26 MST
**File:** crypto_intel.py (2015 lines, 76603 bytes)
...
def _crypto_intel_qty(signal, base_qty=50):
    """Compute trade quantity for the crypto_intel paper strategy.

    Scales base_qty by confidence and applies SIZE_MULTIPLIER_BY_SIDE so paper
    sizing matches prod (half-size on YES, full-size on NO). Clipped to [10, 200].
    Centralized so paper and prod cannot drift again.
    """
    conf = signal.get("confidence", 0.5) or 0.5
    side_mult = SIZE_MULTIPLIER_BY_SIDE.get(signal.get("side"), 1.0)
    return max(10, min(int(base_qty * conf * 3 * side_mult), 200))
...
MIN_EDGE_BY_COIN = {
    'BTC':  0.10,
    'BNB':  0.99,  # disabled 2026-05-18: 39 trades / 30d, both sides ~−$50,
                   # hit rates ≈ implied probabilities (no edge). Set to 0.99
                   # to soft-disable until BNB-specific issue is diagnosed.
                   # Revert to 0.08 to re-enable.
    'HYPE': 0.08,
    'ETH':  0.04,
    'SOL':  0.04,
    'XRP':  0.04,
...
def already_have_open_market_position(market_ticker):
    ...
    Returns True if any crypto_intel-family strategy already has an open or
    pending-settlement position on this market. Conservative: a position in
    any of the three strategies blocks new entries from any caller, which
    prevents same-market stacking within a strategy and cross-strategy
    double-exposure on the same outcome.
    """
...
def _execute_trade(market, signal):
    ...
    try:
        ...
        try:
            ...
            # Scale position by confidence and apply side multiplier
            qty = _crypto_intel_qty(signal)
            cost = round(price * qty, 2)
